# ============================================
# REDES NEURONALES MLP - OPCIONES CALL
...
if (!dir.exists(RUTA_CALL)) dir.create(RUTA_CALL, recursive = TRUE)
...
archivo_progreso <- file.path(carpeta_tablas, "progreso_grid_search_CALL.xlsx")
...
cat("\n===========================================\n")
cat("LAS 3 MEJORES REDES NEURONALES (CALL)\n")
cat("===========================================\n\n")
...
write_xlsx(resultados, file.path(carpeta_tablas, "900_configuraciones_MLP_CALL.xlsx"))
write_xlsx(top3, file.path(carpeta_tablas, "top3_MLP_CALL.xlsx"))

comparacion <- data.frame(
  Modelo = c("BS Original", "Baseline (C=+$10.28)", "Regresión Lineal",
             "Random Forest", "XGBoost", "KNN",
             paste0("Red Neuronal ", LETTERS[1:3])),
  RMSE = c(98.42, 97.66, 73.15, 18.67, 17.20, 17.42,
           top3$rmse_test[1], top3$rmse_test[2], top3$rmse_test[3]),
  MAE = c(25.96, 32.33, 30.71, 7.42, 6.79, 6.28,
          top3$mae_test[1], top3$mae_test[2], top3$mae_test[3])
) %>%
  mutate(Mejora_vs_Baseline = round((1 - RMSE / 97.66) * 100, 2))

write_xlsx(comparacion, file.path(carpeta_tablas, "comparacion_modelos_MLP_CALL.xlsx"))
...
p1 <- ggplot(resultados, aes(x = rmse_test)) +
  geom_histogram(bins = 30, fill = color_call, alpha = 0.7, color = "white") +
  geom_vline(xintercept = top3$rmse_test[1], linetype = "dashed", color = "darkred", linewidth = 1) +
  annotate("text", x = top3$rmse_test[1] + 2, y = Inf, vjust = 2,
           label = paste0("Mejor: $", round(top3$rmse_test[1], 2)), color = "darkred", size = 4) +
  labs(
    title = "Distribución de RMSE - 900 Configuraciones MLP (CALL)",
...
ggsave(file.path(carpeta_graficos, "fig_4-6-1_distribucion_rmse_900_call.png"),
       p1, width = 10, height = 6, dpi = 300)

# RMSE por arquitectura
p2 <- ggplot(resultados, aes(x = reorder(arquitectura, rmse_test), y = rmse_test)) +
  geom_boxplot(fill = color_call, alpha = 0.7) +
  coord_flip() +
  labs(
    title = "RMSE por Arquitectura - MLP (CALL)",
...
ggsave(file.path(carpeta_graficos, "fig_4-6-1_rmse_arquitectura_call.png"),
...
p3 <- ggplot(resultados, aes(x = factor(dropout), y = rmse_test)) +
  geom_boxplot(fill = color_call, alpha = 0.7) +
  labs(
    title = "RMSE por Tasa de Dropout - MLP (CALL)",
    x = "Dropout",
    y = "RMSE Test ($)"
  ) +
  tema_tesis

ggsave(file.path(carpeta_graficos, "fig_4-6-1_rmse_dropout_call.png"),
...
p4 <- ggplot(resultados, aes(x = factor(learning_rate), y = rmse_test)) +
  geom_boxplot(fill = color_call, alpha = 0.7) +
  labs(
    title = "RMSE por Learning Rate - MLP (CALL)",
    x = "Learning Rate",
    y = "RMSE Test ($)"
  ) +
  tema_tesis

ggsave(file.path(carpeta_graficos, "fig_4-6-1_rmse_learning_rate_call.png"),
...
p5 <- ggplot(df_comp, aes(x = Modelo, y = RMSE)) +
  geom_bar(stat = "identity",
           fill = c("#95A5A6", "#95A5A6", "#3498DB", "#27AE60", "#9B59B6", "#E74C3C",
                    color_call, color_call, color_call),
           alpha = 0.8) +
...
ggsave(file.path(carpeta_graficos, "fig_4-6-1_comparacion_modelos_call.png"),
...
p6 <- ggplot(top3_plot, aes(x = Red, y = rmse_test)) +
  ...
  labs(
    title = "Top 3 Redes Neuronales - RMSE (CALL)",
    subtitle = "Grid Search Completo (900 configuraciones)",
    x = NULL,
    y = "RMSE Test ($)"
  ) +
...
ggsave(file.path(carpeta_graficos, "fig_4-6-1_top3_call.png"),
...
cat("\n===========================================\n")
cat("MLP CALL - GRID SEARCH COMPLETO\n")
cat("===========================================\n\n")
...
mejora_vs_baseline <- (1 - top3$rmse_test[1] / 97.66) * 100
...
# ============================================================
archivo_metricas <- "metricas_MLP_CALL.txt"
sink(archivo_metricas)
cat("=============================================================\n")
cat("MÉTRICAS - RED NEURONAL MLP - OPCIONES CALL\n")
cat("=============================================================\n\n")
