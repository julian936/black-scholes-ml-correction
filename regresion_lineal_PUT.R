# ============================================
# REGRESIÓN LINEAL MÚLTIPLE - OPCIONES PUT
...
cat("====================================\n")
cat("REGRESIÓN LINEAL MÚLTIPLE - PUT\n")
cat("====================================\n\n")
...
vars_disponibles <- intersect(variables_modelo, names(train_put))
vars_faltantes <- setdiff(variables_modelo, names(train_put))
...
modelo_lm_put <- lm(formula_lm, data = train_put)

# Resumen del modelo
cat("\n--- RESUMEN DEL MODELO ---\n")
summary_modelo <- summary(modelo_lm_put)
...
write_xlsx(coeficientes, file.path(RUTA_PUT, "tabla_4-X_coeficientes_lm_PUT.xlsx"))
...
residuos <- residuals(modelo_lm_put)
valores_ajustados <- fitted(modelo_lm_put)
residuos_estandarizados <- rstandard(modelo_lm_put)
...
p_linealidad <- ggplot(df_residuos, aes(x = Ajustados, y = Residuos)) +
  ...
  labs(
    title = "Residuos vs Valores Ajustados - PUT",
...
ggsave(file.path(RUTA_PUT, "fig_4-X_linealidad_PUT.png"),
       p_linealidad, width = 10, height = 7, dpi = 300)
cat("   Gráfico guardado: fig_4-X_linealidad_PUT.png\n\n")

# --- HOMOCEDASTICIDAD ---
cat("2. HOMOCEDASTICIDAD\n")

# Test de Breusch-Pagan
bp_test <- bptest(modelo_lm_put)
...
ggsave(file.path(RUTA_PUT, "fig_4-X_homocedasticidad_PUT.png"),
       p_homocedasticidad, width = 10, height = 7, dpi = 300)
cat("   Gráfico guardado: fig_4-X_homocedasticidad_PUT.png\n\n")
...
p_normalidad <- ggplot(df_qq, aes(sample = Residuos)) +
  ...
  labs(
    title = "Q-Q Plot de Residuos Estandarizados - PUT",
...
ggsave(file.path(RUTA_PUT, "fig_4-X_normalidad_PUT.png"),
       p_normalidad, width = 10, height = 7, dpi = 300)
cat("   Gráfico guardado: fig_4-X_normalidad_PUT.png\n\n")
...
tryCatch({
  ...
  if (is.matrix(vif_valores)) {
    ...
    # Guardar VIF
    write_xlsx(vif_df, file.path(RUTA_PUT, "tabla_4-X_vif_PUT.xlsx"))
  } else {
    vif_df <- data.frame(
...
write_xlsx(supuestos_resumen, file.path(RUTA_PUT, "tabla_4-X_supuestos_PUT.xlsx"))
...
pred_train <- predict(modelo_lm_put, newdata = train_put)
pred_test <- predict(modelo_lm_put, newdata = test_put)

# Métricas de entrenamiento
rmse_train <- sqrt(mean((train_put$Diferencia - pred_train)^2, na.rm = TRUE))
mae_train <- mean(abs(train_put$Diferencia - pred_train), na.rm = TRUE)
r2_train <- summary_modelo$r.squared
r2_adj_train <- summary_modelo$adj.r.squared

# Métricas de prueba
rmse_test <- sqrt(mean((test_put$Diferencia - pred_test)^2, na.rm = TRUE))
mae_test <- mean(abs(test_put$Diferencia - pred_test), na.rm = TRUE)

# R² en test
ss_res_test <- sum((test_put$Diferencia - pred_test)^2, na.rm = TRUE)
ss_tot_test <- sum((test_put$Diferencia - mean(test_put$Diferencia, na.rm = TRUE))^2, na.rm = TRUE)
r2_test <- 1 - (ss_res_test / ss_tot_test)

# Calcular baseline (modelo constante C)
C_baseline <- mean(train_put$Diferencia, na.rm = TRUE)
pred_baseline_test <- rep(C_baseline, nrow(test_put))
rmse_baseline <- sqrt(mean((test_put$Diferencia - pred_baseline_test)^2, na.rm = TRUE))
mae_baseline <- mean(abs(test_put$Diferencia - pred_baseline_test), na.rm = TRUE)
...
cat("  N observaciones:", nrow(train_put), "\n")
...
cat("  N observaciones:", nrow(test_put), "\n")
...
# Guardar tablas
write_xlsx(tabla_metricas, file.path(RUTA_PUT, "tabla_4-X_metricas_lm_PUT.xlsx"))
write_xlsx(tabla_mejora, file.path(RUTA_PUT, "tabla_4-X_mejora_vs_baseline_PUT.xlsx"))
...
test_put$Pred_LM <- pred_test
test_put$Precio_Ajustado_LM <- test_put$Precio_BS + pred_test
test_put$Error_Final_LM <- test_put$Last - test_put$Precio_Ajustado_LM

# Comparar errores
error_bs_original <- test_put$Last - test_put$Precio_BS  # = Diferencia
error_lm_ajustado <- test_put$Error_Final_LM
...
write_xlsx(tabla_errores_finales, file.path(RUTA_PUT, "tabla_4-X_errores_finales_PUT.xlsx"))

# Ejemplos de ajustes
cat("\n--- EJEMPLOS DE AJUSTES ---\n")
set.seed(42)
ejemplos_idx <- sample(1:nrow(test_put), min(10, nrow(test_put)))
ejemplos <- test_put[ejemplos_idx, c("Ticker", "Strike", "Precio_Actual",
...
write_xlsx(ejemplos, file.path(RUTA_PUT, "tabla_4-X_ejemplos_ajuste_PUT.xlsx"))
...
p_pred_vs_real <- ggplot(data.frame(Real = test_put$Diferencia, Predicho = pred_test),
                         aes(x = Real, y = Predicho)) +
  geom_point(alpha = 0.4, color = "#3498DB", size = 1) +
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed", linewidth = 1) +
  labs(
    title = "Predicciones vs Valores Reales - Regresión Lineal PUT",
...
ggsave(file.path(RUTA_PUT, "fig_4-X_pred_vs_real_PUT.png"),
       p_pred_vs_real, width = 10, height = 10, dpi = 300)
cat("  Gráfico guardado: fig_4-X_pred_vs_real_PUT.png\n")
...
p_dist_errores <- ggplot(df_errores_comp, aes(x = Error, fill = Modelo)) +
  ...
  labs(
    title = "Distribución de Errores: BS Original vs BS Ajustado - PUT",
...
ggsave(file.path(RUTA_PUT, "fig_4-X_dist_errores_PUT.png"),
       p_dist_errores, width = 12, height = 8, dpi = 300)
cat("  Gráfico guardado: fig_4-X_dist_errores_PUT.png\n")
...
p_hist_residuos <- ggplot(data.frame(Residuos = residuos), aes(x = Residuos)) +
  ...
  labs(
    title = "Distribución de Residuos - Regresión Lineal PUT",
...
ggsave(file.path(RUTA_PUT, "fig_4-X_hist_residuos_PUT.png"),
       p_hist_residuos, width = 10, height = 7, dpi = 300)
cat("  Gráfico guardado: fig_4-X_hist_residuos_PUT.png\n")
...
p_coef <- ggplot(coef_plot_t, aes(x = reorder(Variable, t_abs), y = t_abs)) +
  ...
  coord_flip() +
  labs(
    title = "Top 10 Variables por Significancia Estadística - PUT",
...
ggsave(file.path(RUTA_PUT, "fig_4-X_importancia_coef_PUT.png"),
       p_coef, width = 10, height = 8, dpi = 300)
cat("  Gráfico guardado: fig_4-X_importancia_coef_PUT.png\n")
...
write_xlsx(test_put, file.path(RUTA_PUT, "resultados_test_lm_PUT.xlsx"))
cat("  resultados_test_lm_PUT.xlsx\n")
...
resumen_ejecutivo <- data.frame(
  ...
  Valor = c(
    nrow(train_put),
    nrow(test_put),
    length(coef(modelo_lm_put)) - 1,  # Excluir intercepto
    round(r2_train, 4),
    round(r2_test, 4),
    round(rmse_train, 4),
    round(rmse_test, 4),
...
write_xlsx(resumen_ejecutivo, file.path(RUTA_PUT, "resumen_ejecutivo_lm_PUT.xlsx"))
cat("  resumen_ejecutivo_lm_PUT.xlsx\n")

# Guardar modelo (para uso posterior)
saveRDS(modelo_lm_put, file.path(RUTA_PUT, "modelo_lm_PUT.rds"))
cat("  modelo_lm_PUT.rds\n")
...
cat("\n===========================================\n")
cat("RESUMEN FINAL - REGRESIÓN LINEAL PUT\n")
cat("===========================================\n\n")

cat("Observaciones de entrenamiento:", nrow(train_put), "\n")
cat("Observaciones de prueba:", nrow(test_put), "\n")
cat("Variables explicativas:", length(coef(modelo_lm_put)) - 1, "\n\n")
...
cat("ARCHIVOS GENERADOS EN:", RUTA_PUT, "\n")
cat("  - tabla_4-X_coeficientes_lm_PUT.xlsx\n")
cat("  - tabla_4-X_vif_PUT.xlsx\n")
cat("  - tabla_4-X_supuestos_PUT.xlsx\n")
cat("  - tabla_4-X_metricas_lm_PUT.xlsx\n")
cat("  - tabla_4-X_mejora_vs_baseline_PUT.xlsx\n")
cat("  - tabla_4-X_errores_finales_PUT.xlsx\n")
cat("  - tabla_4-X_ejemplos_ajuste_PUT.xlsx\n")
cat("  - resumen_ejecutivo_lm_PUT.xlsx\n")
cat("  - resultados_test_lm_PUT.xlsx\n")
cat("  - modelo_lm_PUT.rds\n")
cat("  - fig_4-X_linealidad_PUT.png\n")
cat("  - fig_4-X_homocedasticidad_PUT.png\n")
cat("  - fig_4-X_normalidad_PUT.png\n")
cat("  - fig_4-X_pred_vs_real_PUT.png\n")
cat("  - fig_4-X_dist_errores_PUT.png\n")
cat("  - fig_4-X_hist_residuos_PUT.png\n")
cat("  - fig_4-X_importancia_coef_PUT.png\n")
...
# ============================================================
archivo_metricas <- "metricas_regresion_lineal_PUT.txt"
sink(archivo_metricas)
cat("=============================================================\n")
cat("MÉTRICAS - REGRESIÓN LINEAL - OPCIONES PUT\n")
cat("=============================================================\n\n")
...
cat("N variables predictoras:", length(coef(modelo_lm_put)) - 1, "\n\n")
