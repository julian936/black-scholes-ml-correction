# ============================================
# XGBOOST - OPCIONES CALL (con Ticker)
...
RUTA_SALIDA <- "C:/Users/julia/OneDrive/Documentos/bookdown-demo-main/objetivo 2/XGBoost/Call"
...
dummy_train <- model.matrix(~ Ticker - 1, data = train_call)
dummy_test <- model.matrix(~ Ticker - 1, data = test_call)

# Preparar matrices para XGBoost
X_train <- as.matrix(cbind(train_call[, variables_numericas], dummy_train))
X_test <- as.matrix(cbind(test_call[, variables_numericas], dummy_test))
y_train <- train_call[[variable_objetivo]]
y_test <- test_call[[variable_objetivo]]
...
write_xlsx(resultados_cv, file.path(carpeta_tablas, "optimizacion_hiperparametros_CALL.xlsx"))
...
pred_train <- predict(modelo_xgb_call, dtrain)
pred_test <- predict(modelo_xgb_call, dtest)

# Agregar predicciones al test set
test_call$Pred_XGB <- pred_test
test_call$Precio_XGB <- test_call$Precio_BS + pred_test
test_call$Error_XGB <- test_call$Diferencia - pred_test
...
mejora_individual <- abs(test_call$Last - test_call$Precio_XGB) < abs(test_call$Diferencia)
...
importancia <- xgb.importance(model = modelo_xgb_call)
...
# Guardar importancia
write_xlsx(as.data.frame(importancia), file.path(carpeta_tablas, "importancia_variables_CALL.xlsx"))
...
write_xlsx(metricas, file.path(carpeta_tablas, "metricas_xgb_CALL.xlsx"))

# Comparación con otros modelos
comparacion <- data.frame(
  Modelo = c("BS Original", "Baseline (C=+$10.28)", "Regresión Lineal", "Random Forest", "XGBoost"),
  RMSE = c(98.42, 97.66, 73.15, 17.38, rmse_test),
  MAE = c(25.96, 32.33, 30.71, 5.79, mae_test)
) %>%
...
write_xlsx(test_call, file.path(carpeta_tablas, "resultados_test_xgb_CALL.xlsx"))

# Guardar modelo
xgb.save(modelo_xgb_call, file.path(RUTA_SALIDA, "modelo_xgb_CALL.model"))
...
write_xlsx(resumen, file.path(carpeta_tablas, "resumen_ejecutivo_xgb_CALL.xlsx"))
...
eval_log <- modelo_xgb_call$evaluation_log
...
p1_evolucion <- ggplot(df_evolucion, aes(x = Iteracion, y = RMSE, color = Conjunto)) +
  ...
  labs(
    title = "Evolución del Error durante Entrenamiento - XGBoost (CALL)",
...
ggsave(file.path(carpeta_graficos, "fig_evolucion_error_CALL.png"),
...
p2_importancia <- ggplot(df_imp_plot, aes(x = Feature, y = Importancia_Normalizada)) +
  ...
  labs(
    title = "Importancia de Variables - XGBoost (CALL)",
...
ggsave(file.path(carpeta_graficos, "fig_importancia_variables_CALL.png"),
...
p3_pred_vs_real <- ggplot(df_pred_plot, aes(x = Real, y = Predicho)) +
  ...
  labs(
    title = "Predicciones vs Valores Reales - XGBoost (CALL)",
...
ggsave(file.path(carpeta_graficos, "fig_pred_vs_real_CALL.png"),
...
p4_residuos <- ggplot(df_residuos, aes(x = Residuo)) +
  geom_histogram(aes(y = after_stat(density)), bins = 50, 
                 fill = "#E67E22", alpha = 0.7, color = "white") +
  ...
  labs(
    title = "Distribución de Residuos - XGBoost (CALL)",
...
ggsave(file.path(carpeta_graficos, "fig_distribucion_residuos_CALL.png"),
...
p5_comparacion <- ggplot(df_comparacion, aes(x = Modelo, y = RMSE, fill = Modelo)) +
  ...
  scale_fill_manual(values = c("#E74C3C", "#F39C12", "#3498DB", "#27AE60", "#E67E22")) +
  scale_y_continuous(limits = c(0, 110)) +
  labs(
    title = "Comparación de RMSE entre Modelos - Opciones CALL",
...
ggsave(file.path(carpeta_graficos, "fig_comparacion_modelos_CALL.png"),
...
# ============================================================
archivo_metricas <- "metricas_xgboost_CALL.txt"
sink(archivo_metricas)
cat("=============================================================\n")
cat("MÉTRICAS - XGBOOST - OPCIONES CALL\n")
cat("=============================================================\n\n")
