# ============================================
# RANDOM FOREST - OPCIONES CALL (con Ticker)
...
cat("====================================\n")
cat("RANDOM FOREST - OPCIONES CALL\n")
cat("====================================\n\n")
...
vars_faltantes <- setdiff(variables_predictoras, names(train_call))
...
cat("Niveles de Ticker:", paste(levels(train_call$Ticker), collapse = ", "), "\n\n")

# Preparar datasets para Random Forest
train_rf <- train_call[, c(variable_objetivo, variables_predictoras)]
test_rf <- test_call[, c(variable_objetivo, variables_predictoras)]
...
write_xlsx(resultados_mtry, file.path(carpeta_tablas, "optimizacion_mtry_CALL.xlsx"))
...
# Modelo final (usar 200 árboles si hay convergencia temprana)
mejor_ntree <- 200
cat("\nEntrenando modelo final con ntree=", mejor_ntree, ", mtry=", mejor_mtry, "...\n")

modelo_rf_call <- randomForest(
  Diferencia ~ .,
  data = train_rf,
  ntree = mejor_ntree,
  mtry = mejor_mtry,
...
pred_train <- predict(modelo_rf_call, train_rf)
pred_test <- predict(modelo_rf_call, test_rf)

# Agregar predicciones al test set
test_call$Pred_RF <- pred_test
test_call$Precio_RF <- test_call$Precio_BS + pred_test
test_call$Error_RF <- test_call$Diferencia - pred_test
...
rmse_bs_original <- sqrt(mean(test_call$Diferencia^2))
rmse_rf_precio <- sqrt(mean((test_call$Last - test_call$Precio_RF)^2))
...
mejora_individual <- abs(test_call$Last - test_call$Precio_RF) < abs(test_call$Diferencia)
...
importancia <- importance(modelo_rf_call)
...
# Guardar importancia
write_xlsx(importancia_df, file.path(carpeta_tablas, "importancia_variables_CALL.xlsx"))
...
write_xlsx(metricas, file.path(carpeta_tablas, "metricas_rf_CALL.xlsx"))

# Comparación con otros modelos
comparacion <- data.frame(
  Modelo = c("BS Original", "Baseline (C=+$10.28)", "Regresión Lineal", "Random Forest"),
  RMSE = c(98.42, 97.66, 73.15, rmse_test),
  MAE = c(25.96, 32.33, 30.71, mae_test)
) %>%
...
write_xlsx(test_call, file.path(carpeta_tablas, "resultados_test_rf_CALL.xlsx"))

# Guardar modelo
saveRDS(modelo_rf_call, file.path(RUTA_CALL, "modelo_rf_CALL.rds"))
...
write_xlsx(resumen, file.path(carpeta_tablas, "resumen_ejecutivo_rf_CALL.xlsx"))
...
df_convergencia <- data.frame(
  ...
  RMSE_OOB = sqrt(modelo_rf_call$mse)
)
...
p1_convergencia <- ggplot(df_convergencia, aes(x = Arboles, y = RMSE_OOB)) +
  ...
  annotate("text", x = mejor_ntree * 0.75, y = rmse_final * 1.03,
           ...
           color = "red", size = 3.5) +
  labs(
    title = "Convergencia del Error OOB - Random Forest (CALL)",
...
ggsave(file.path(carpeta_graficos, "fig_convergencia_oob_CALL.png"),
...
p2_importancia <- ggplot(df_imp_plot, aes(x = Variable, y = Importancia_Normalizada)) +
  ...
  labs(
    title = "Importancia de Variables - Random Forest (CALL)",
...
p3_pred_vs_real <- ggplot(df_pred_plot, aes(x = Real, y = Predicho)) +
  ...
  labs(
    title = "Predicciones vs Valores Reales - Random Forest (CALL)",
...
p4_residuos <- ggplot(df_residuos, aes(x = Residuo)) +
  ...
  labs(
    title = "Distribución de Residuos - Random Forest (CALL)",
...
p5_tuning_mtry <- ggplot(resultados_mtry, aes(x = factor(mtry), y = OOB_RMSE)) +
  ...
  labs(
    title = "Optimización de mtry - Random Forest (CALL)",
...
ggsave(file.path(carpeta_graficos, "fig_tuning_mtry_CALL.png"),
...
cat("\n===========================================\n")
cat("RANDOM FOREST CALL COMPLETADO\n")
cat("===========================================\n\n")
...
# ============================================================
archivo_metricas <- "metricas_randomForest_CALL.txt"
sink(archivo_metricas)
cat("=============================================================\n")
cat("MÉTRICAS - RANDOM FOREST - OPCIONES CALL\n")
cat("=============================================================\n\n")
