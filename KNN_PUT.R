# ============================================
# K-NEAREST NEIGHBORS (KNN) - OPCIONES PUT
...
cat("====================================\n")
cat("KNN - OPCIONES PUT (con Ticker)\n")
cat("====================================\n\n")
...
vars_faltantes <- setdiff(variables_numericas, names(train_put))
...
write_xlsx(stats_norm, file.path(carpeta_tablas, "normalizacion_stats_PUT.xlsx"))
...
write_xlsx(resultados_k, file.path(carpeta_tablas, "optimizacion_k_PUT.xlsx"))
...
pred_train <- predict(modelo_knn_put, X_train)
pred_test <- predict(modelo_knn_put, X_test)

# Agregar predicciones al test set original
test_put_results <- test_put[complete.cases(test_put[, c(variable_objetivo, variables_numericas)]), ]
test_put_results$Pred_KNN <- pred_test
test_put_results$Precio_KNN <- test_put_results$Precio_BS + pred_test
test_put_results$Error_KNN <- test_put_results$Diferencia - pred_test
...
rmse_bs_original <- sqrt(mean(test_put_results$Diferencia^2))
rmse_knn_precio <- sqrt(mean((test_put_results$Last - test_put_results$Precio_KNN)^2))
...
mejora_individual <- abs(test_put_results$Last - test_put_results$Precio_KNN) < abs(test_put_results$Diferencia)
...
for (i in seq_along(variables_predictoras)) {
  ...
  pred_perm <- predict(modelo_knn_put, X_test_perm)
...
# Guardar importancia
write_xlsx(importancia_perm, file.path(carpeta_tablas, "importancia_variables_PUT.xlsx"))
...
write_xlsx(metricas, file.path(carpeta_tablas, "metricas_knn_PUT.xlsx"))

# Comparación con otros modelos
comparacion <- data.frame(
  Modelo = c("BS Original", "Baseline (C=-$16.34)", "Regresión Lineal", "Random Forest", "XGBoost", "KNN"),
  RMSE = c(105.02, 103.54, 31.43, 11.18, 10.52, rmse_test),
  MAE = c(22.78, 34.06, 16.02, 4.53, 4.12, mae_test)
) %>%
...
write_xlsx(test_put_results, file.path(carpeta_tablas, "resultados_test_knn_PUT.xlsx"))

# Guardar modelo
saveRDS(modelo_knn_put, file.path(RUTA_PUT, "modelo_knn_PUT.rds"))
...
saveRDS(params_norm, file.path(RUTA_PUT, "normalizacion_params_PUT.rds"))
...
write_xlsx(resumen, file.path(carpeta_tablas, "resumen_ejecutivo_knn_PUT.xlsx"))
...
p1_tuning_k <- ggplot(resultados_k, aes(x = k, y = RMSE_CV)) +
  geom_line(color = color_put, linewidth = 1) +
  geom_point(color = color_put, size = 3) +
  ...
  labs(
    title = "Optimización del Hiperparámetro k - KNN (PUT)",
...
ggsave(file.path(carpeta_graficos, "fig_4-5-1_tuning_k_put.png"),
...
p2_importancia <- ggplot(df_imp_plot, aes(x = Variable, y = Importancia_Normalizada)) +
  ...
  labs(
    title = "Importancia de Variables - KNN (PUT)",
...
ggsave(file.path(carpeta_graficos, "fig_4-5-1_importancia_variables_put.png"),
...
p3_pred_vs_real <- ggplot(df_pred_plot, aes(x = Real, y = Predicho)) +
  geom_point(alpha = 0.4, color = color_put, size = 1.5) +
  ...
  labs(
    title = "Predicciones vs Valores Reales - KNN (PUT)",
...
ggsave(file.path(carpeta_graficos, "fig_4-5-1_pred_vs_real_put.png"),
...
p4_residuos <- ggplot(df_residuos, aes(x = Residuo)) +
  geom_histogram(aes(y = after_stat(density)), bins = 50,
                 fill = color_put, alpha = 0.7, color = "white") +
  ...
  labs(
    title = "Distribución de Residuos - KNN (PUT)",
...
ggsave(file.path(carpeta_graficos, "fig_4-5-1_distribucion_residuos_put.png"),
...
p5_comparacion <- ggplot(df_comparacion, aes(x = Modelo, y = RMSE)) +
  geom_bar(stat = "identity", fill = c("#95A5A6", "#95A5A6", "#3498DB", "#27AE60", "#9B59B6", color_put), alpha = 0.8) +
...
ggsave(file.path(carpeta_graficos, "fig_4-5-1_error_por_ticker_put.png"),
...
cat("\n===========================================\n")
cat("KNN PUT COMPLETADO (con Ticker)\n")
cat("===========================================\n\n")
...
# ============================================================
archivo_metricas <- "metricas_KNN_PUT.txt"
sink(archivo_metricas)
cat("=============================================================\n")
cat("MÉTRICAS - KNN - OPCIONES PUT\n")
cat("=============================================================\n\n")
